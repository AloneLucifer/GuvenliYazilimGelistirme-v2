...
if ($role == 'admin' || $role == 'editor') {
    $recipes_query = "SELECT id, title, description, image FROM recipes WHERE title LIKE '%$query%' OR description LIKE '%$query%'";
} else {
    $recipes_query = "SELECT id, title, description, image FROM recipes WHERE (title LIKE '%$query%' OR description LIKE '%$query%') AND is_private = 0";
}

// SQL Injection zafiyetini test etmek için zararlı sorgu
...
if (!$recipes_result) {
    die('Query Error: ' . $conn->lastErrorMsg());
}
?>
