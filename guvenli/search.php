...
if ($role == 'admin' || $role == 'editor') {
    $stmt = $conn->prepare("SELECT id, title, description, image FROM recipes WHERE title LIKE ? OR description LIKE ?");
    $search_query = '%' . $query . '%';
    $stmt->bindValue(1, $search_query, SQLITE3_TEXT);
    $stmt->bindValue(2, $search_query, SQLITE3_TEXT);
} else {
    $stmt = $conn->prepare("SELECT id, title, description, image FROM recipes WHERE (title LIKE ? OR description LIKE ?) AND is_private = 0");
    $search_query = '%' . $query . '%';
    $stmt->bindValue(1, $search_query, SQLITE3_TEXT);
    $stmt->bindValue(2, $search_query, SQLITE3_TEXT);
}

$recipes_result = $stmt->execute();
